/**
 * The HTTP plumbing the two host route modules share.
 *
 * Every JSON route this plugin registers answers one of two things: a domain
 * value, or a bounded JSON request body. Both route modules writing those
 * helpers would be two places to fix the next time the envelope, the cache
 * header, or the body cap moves — so they live here.
 *
 * @module my-sider/host/http
 */
