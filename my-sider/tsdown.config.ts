/**
 * Build config for `my-sider`: two artifacts from one package.
 *
 * The in-repo preset that emits the browser artifact (`packages/client/tsdown.client.ts`)
 * is not published, so this file reproduces its output contract exactly:
 *
 *   - the client bundle is a CLASSIC SCRIPT that registers a closure factory:
 ...
 *     script execution
 *
 * The host half is plain ESM for Node. Every `@deepseek-ai/*` import in it is
 * type-only and erased here, so the artifact has no runtime dependencies at
 * all — which is what lets the Loader resolve this package's bare name from the
 * profile directory without installing anything.
 */
 ...
 * specifiers stay imports; a bundle that inlined React would hold a second React
 * instance and break hooks.
 */
...
const clientHalf = {
  ...
  plugins: [{
    ...
    name: 'my-sider-client-bundle-purity',
